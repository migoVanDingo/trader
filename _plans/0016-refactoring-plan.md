...
- [ ] **1 Extract `useClickOutside` (or `usePopover`) hook.**
...
- [ ] **2 Move `formatQty` into `lib/format.ts`.**
...
- [ ] **3 Use selectors in `IndicatorMenu`.**
...
- [ ] **4 Extract a stream-subscription helper for data hooks.**
...
- [ ] **5 Consider a chart-series lifecycle helper.**
...
- [ ] **7 `indicators.ts` split (only if it grows).**
      If it crosses ~400 lines, split into `lib/indicators/` (`movingAverages.ts`,
      `bands.ts`, `oscillators.ts`, `macd.ts`, `merge.ts`) with a barrel `index.ts`.
      Not needed today.
- [ ] **8 Store toggle helpers (optional).**
